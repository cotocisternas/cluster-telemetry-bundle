...
| M7 | Single-tier collector also runs Target-Allocator Prometheus scraping — scrape load, OTLP ingest, and tail sampling share one workload and scaling domain; TA scrape collectors conventionally want stable StatefulSet identity | `components/prometheus-crd-scrape/otel-collector-values.yaml` | Split a dedicated StatefulSet scrape collector fed by the TA, distinct from the OTLP ingest Deployment |
...
| M9 | **`presentation` profile** (dashboards, PrometheusRule, KEDA examples) referenced in three docs but **not implemented** | `README.md:89`, `docs/monitoring-bundle-profiles.md:55-150` | Ship `components/presentation` as examples-first (Grafana JSON, sample rules, sample KEDA ScaledObject keyed on `cluster`) |
